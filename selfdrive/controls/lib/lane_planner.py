...
class LanePlanner:
  ...
  def parse_model(self, md):
    ...
    if len(edges[0].t) == TRAJECTORY_SIZE:
      self.le_y = np.array(edges[0].y) + clamp(md.roadEdgeStds[0] * 0.5, 0, 0.75) + LANE_OFFSET
      self.re_y = np.array(edges[1].y) - clamp(md.roadEdgeStds[1] * 0.5, 0, 0.75) + LANE_OFFSET
    else:
