...
from selfdrive.car.ford_bp.fordcan import CanBus
...
class RadarInterface(RadarInterfaceBase):
  def __init__(self, CP):
    ...
    self.track_id = 0
    self.radar = DBC[CP.carFingerprint].get("radar")
  ...
  def _update_steer_assist_data(self):
    ...
    if confidence > 0:
      if 0 not in self.pts:
        self.pts[0] = car.RadarData.RadarPoint()
  ...
  def _update_delphi_mrr(self, ret: car.RadarData):
  ...
  def _update_delphi_mrr_64(self, ret: car.RadarData):
  ...
  def do_clustering(self):
    ...
    for idx, (track_id, pts) in enumerate(points_by_track_id.items()):
      ...
      if idx not in self.pts:
        self.pts[idx] = car.RadarData.RadarPoint(measured=True, aRel=float('nan'), yvRel=float('nan'))
