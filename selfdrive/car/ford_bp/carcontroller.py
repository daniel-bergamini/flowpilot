...
def apply_ford_curvature_limits(apply_curvature, apply_curvature_last, current_curvature, v_ego_raw, CP):
...
class CarController:
  ...
  def update(self, CC, sm, CS, now_nanos):
    ...
    if sm is not None:
      if 'modelV2' in sm:
        self.model = sm['modelV2']
      if 'liveParameters' in sm and sm.updated.get('liveParameters'):
        self.lp = sm['liveParameters']
      if 'selfdriveState' in sm and sm.updated.get('selfdriveState'):
        self.ss = sm['selfdriveState']
    ...
    if self.send_driver_monitor_can_msg and self.ss is not None:
    ...
    if (self.frame % CarControllerParams.STEER_STEP) == 0:
      if CC.latActive:
        ...
        if self.model is not None and len(self.model.orientation.x) >= 17:
          ...
          predicted_steering_angle_curvature = interp(self.wheel_angle_lookup_time, T_IDXS, curvatures)
          predicted_curvature = interp(self.curvature_lookup_time, T_IDXS, curvatures)
        else:
        ...
        if self.post_reset_ramp_active:
          ...
          apply_curvature = apply_std_steer_angle_limits(
            requested_curvature, self.apply_curvature_last, CS.out.vEgoRaw, CarControllerParams
          )
        ...
        path_offset_position = interp(self.path_offset_lookup_time, T_IDXS, self.model.position.y)
        ...
        if self.disable_BP_lat_UI:
          ...
          self.apply_curvature_last = apply_ford_curvature_limits(
            apply_curvature, self.apply_curvature_last, current_curvature, CS.out.vEgoRaw, self.CP
          )
