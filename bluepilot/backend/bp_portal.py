...
def _schedule_flowpilot_restart():
    ...
    launch_cmd = f"cd '{BASEDIR}' && scons && flowinit"
    if tmux_path:
        cmd = (
            "sleep 1; "
        ...
        cmd = (
            "sleep 1; "
            "pkill -f flowinit || true; "
            f"nohup sh -lc \"{launch_cmd}\" >/dev/null 2>&1 &"
        )
...
try:
    from selfdrive.loggerd.deleter import MIN_BYTES, MIN_PERCENT
except ImportError:
            ...
            if path == '/api/health':
                ...
                try:
                    ...
                    'network': {
                        ...
                        'port': int(params.get("WebPortalPort") or "8088")
                    },
                    'clients': {
                    ...
                    if action == 'set_param':
                        ...
                        if remove:
                            ...
                            if isinstance(current_keys, bytes):
                                current_keys = current_keys.decode('utf-8', errors='replace')
...
def main():
    ...
    try:
        ...
        # WebPortalPort may not be registered in params schema, use default
        port = 8088
    ...
    # Determine bind address - configurable via params
    ...
    try:
        bind_all = params.get_bool("WebBindAll")
    except Exception:
        bind_all = True
    ...
    if bind_all:
        ...
        bind_address = wifi_ip or '0.0.0.0'
        if wifi_ip is None:
            logger.warning("WebBindAll disabled but no WiFi IP detected; falling back to 0.0.0.0")

    logger.info("=" * 60)
    logger.info("WEB SERVER NETWORK CONFIGURATION")
    logger.info(f"Binding to: {bind_address} (WebBindAll={bind_all})")
